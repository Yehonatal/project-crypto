        ...
        <>
                ...
                {categories.map((category) => (
                    ...
                    >
                        {category.isCustomize ? (
                            ...
                            <span className={activeCategory === category.id ? "text-green-600" : ""}>
                                {category.icon}
                            </span>
                        )}
                        {category.label}
            ...
            {categoryFilteredCoins.length > 0 ? (
                viewMode === 'table' ? (
                    ...
                    <ul className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {categoryFilteredCoins.map((coin: CoinData) => (
                            <li key={coin.id} className="relative bg-white border-2 border-b-5 rounded-2xl border-gray-300 xl-glow p-4 shadow-sm hover:shadow-md transition duration-300 flex flex-col justify-between overflow-hidden">
                ...
                <div className="text-center py-12">
                    <h1 className="text-lg text-gray-600">No coins found</h1>
                    <p className="text-gray-500 mt-2">
                        {activeCategory === 'all' 
                            ? 'Try adjusting your search or filter criteria'
                            : `No coins found in the ${categories.find(c => c.id === activeCategory)?.label} category`
                        }
                    </p>
                </div>
            )}
